...
const livePricing = {
  api: {
    ...
    pollSession: (creds, pageIndex = 0, pageSize = 10) => {
      return fetch(pricingUrl + `/${creds.sessionKey}?apiKey=${config.apiKey}&pageIndex=${pageIndex}&pageSize=${pageSize}`, {
        method: 'GET',
        // uncomment if you'd like to use a development proxy (e.g. Charles or Fiddler)
        // agent: new HttpProxyAgent({
        //   host: 'localhost',
...
function fetchPage (sessionKey, pageIndex) {
  return livePricing.api.pollSession({ sessionKey }, pageIndex)
    .then((response) => {
...
function pollSuccess (state, data) {
  ...
  if (data.Status === 'UpdatesComplete' || state.timedOut) {
    ...
    state.finished = true;
    return state.onFinished({
      ...data,
      sessionKey: state.creds.sessionKey
    });
  }
  state.repoll();
}
...
livePricing.fetchPage = fetchPage;
